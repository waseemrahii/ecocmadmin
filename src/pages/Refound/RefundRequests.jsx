...
const RefundRequests = ({ status, title }) => {
  ...
  useEffect(() => {
    ...
    if (refunds.length > 0) {
      ...
      const filteredData = refunds.filter((refund) =>
        refund.order._id.toLowerCase().includes(lowercasedFilter) ||
        refund.order.products[0]?.name?.toLowerCase().includes(lowercasedFilter) || // Use optional chaining here
        `${refund.order.customer.firstName} ${refund.order.customer.lastName}`.toLowerCase().includes(lowercasedFilter)
      );
      setFilteredRefunds(filteredData);
    } else {
            ...
            >
              ...
              <tbody>
                {filteredRefunds.map((request, index) => {
                  ...
                  const productThumbnail = product.thumbnail || '/image-place-holder.png'; // Default thumbnail
                      ...
                      <td>
                        <div className="d-flex flex-wrap gap-2">
                          ...
                          <div className="d-flex flex-column gap-1">
                            ...
                            <span className="fz-12">QTY: {product.quantity || 0}</span>
                          </div>
                        </div>
                      </td>
                      <td>
